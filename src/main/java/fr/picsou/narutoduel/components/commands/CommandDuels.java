            ...
            if (player.isOp()) {
                if (strings.length == 2) {
                    ...
                    if (strings[0].equals("delete")) {
                        if (config.contains("arena." + strings[1])) {
                            config.set("arena." + strings[1], null);
                            Main.getInstance().saveConfig();
                    ...
                    if (strings[0].equals("setpos1")) {
                        if (config.contains("arena." + strings[1])){
                            config.set("arena."+strings[1]+".x1", player.getLocation().getX());
                            config.set("arena."+strings[1]+".y1", player.getLocation().getY());
                            config.set("arena."+strings[1]+".z1", player.getLocation().getZ());
                            config.set("arena."+strings[1]+".yaw1", player.getLocation().getYaw());
                            config.set("arena."+strings[1]+".pitch1", player.getLocation().getPitch());
                            Main.getInstance().saveConfig();
                            if (config.contains("arena."+strings[1]+".x2")){
                                config.set("arena."+strings[1]+".open", true);
                            }
                    ...
                    if (strings[0].equals("setpos2")) {
                        if (config.contains("arena." + strings[1])){
                            config.set("arena."+strings[1]+".x2", player.getLocation().getX());
                            config.set("arena."+strings[1]+".y2", player.getLocation().getY());
                            config.set("arena."+strings[1]+".z2", player.getLocation().getZ());
                            config.set("arena."+strings[1]+".yaw2", player.getLocation().getYaw());
                            config.set("arena."+strings[1]+".pitch2", player.getLocation().getPitch());
                            Main.getInstance().saveConfig();
                            player.sendMessage(ChatColor.GREEN + "Position 2 définit pour l'arène "+strings[1]);
                            if (config.contains("arena."+strings[1]+".x1")){
                                config.set("arena."+strings[1]+".open", true);
                            }
                            return false;
                        } else{
